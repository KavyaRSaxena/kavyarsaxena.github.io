...
Consider a dataset $$D_1 = \{(X_i,Y_i)\}_{i=1}^{I}$$, where $$X_i$$ is an $$N\times N$$ image and $$Y_i\in \{0,1\}^C$$ is a one-hot vector over $$C=5$$ classes. The ground truth class $$y^*_i$$ is simply obtained by using argmax over the one-hot vector $$Y_i$$. We train the base model $$f_{[\phi,\theta]}$$ on $$D_1$$ dataset. Here, $$\phi$$ and $$\theta$$ are the randomly initialized parameters of the feature extractor layers and
...
Consider a sample $$(X_i,Y_i)\in D_1$$. With $$X_i$$ as an input, the pre-trained base model $$f_{[\phi,\theta]}$$ predicts an output distribution $$\hat{Y}_i$$. 
...
:bulb: **Solution:** True Class Probability is a more suitable confidence criteria
